...
for it in 1:max_iter
    ...
    #                                                                                         (Pdg_max_vary[DG][h,m]-Pdg[DG, h, m]) )
    # for DG in DG_SET
    #     @constraint(OPF, [d in DG_SET, h in HOUR_SET, m in QUARTER_SET], Pdg[d, h, m] <= Pdg[DG, h, m], if Pdg[DG, h, m] <= Pdg_max_vary[d][h,m] end)
    # end
    ...
    # @constraint(OPF, Fairness[h in HOUR_SET, m in QUARTER_SET], fairness_index[h,m] >=
    #                         sum((10-Pdg_max_vary[DG][h,m])*(Pdg_max_vary[DG][h,m]-Pdg[DG, h, m]) for DG in DG_SET))
    ...
    f1 = 1.0323678    # Final PVs - Variable V_break + Prio
    f2 = 26.8814161    # Final PVs - Variable V_break + Prio
    ...
    println(PVC_prnt)
    println("🔹🔸 Each PV Curt. Seprted (MWh): ", [[DG, round.(Sbase*PVC_sep[DG]/4e6, digits=4)] for DG in DG_SET ])
    ...
    if max_diff < tol
        ...
        println("🔸 Scenario: $(senario) ✅ Total Opt. time: $(sum(t_opt)) seconds")
        break
    end

    # global fb1 = [value(fb1[d, h, m]) for d in DG_SET, h in HOUR_SET, m in QUARTER_SET]
    global V_ac_l2 = [value(v[i, h, m]) for i in BUS_SET, h in HOUR_SET, m in QUARTER_SET]
    global Pgen2 = reshape([ value(Pgen[i,h,m]) for i in (slack), h in HOUR_SET, m in QUARTER_SET],
                                                length(slack), length(HOUR_SET), length(QUARTER_SET))
    global Pdg2 = reshape([ value(Pdg[i,h,m]) for i in (DG_SET), h in HOUR_SET, m in QUARTER_SET],
