...
global Vbp_limit = [0.88 1.1] # in p.u.
...
for it in 1:max_iter
    ...
    # @variable(OPF, Pgen[slack, HOUR_SET, QUARTER_SET])  # Active power generation at slack bus
    ...
    @variable(OPF, 0 <= PVC[DG_SET, HOUR_SET, QUARTER_SET])
    ...
    V_break = reshape(repeat([0.88, 0.9, 0.97, 1.0, 1.02, 1.1], 1, length(BUS_SET) * length(HOUR_SET)), 6, length(BUS_SET), length(HOUR_SET))


    # @constraint(OPF, [d in DG_SET, h in HOUR_SET], V_break[1, findfirst(isequal(d), DG_SET[:]), h] == Vbp_limit[1])
    # @constraint(OPF, [d in DG_SET, h in HOUR_SET], V_break[6, findfirst(isequal(d), DG_SET[:]), h] == Vbp_limit[2])
    # Vb_lim = [
    ...
    # @constraint(OPF, [i = 2, d in DG_SET, h in HOUR_SET], V_break[i, findfirst(isequal(d), DG_SET[:]), h] <= V_break[i+1, findfirst(isequal(d), DG_SET[:]), h] - 0.02)
    # @constraint(OPF, [i = 5, d in DG_SET, h in HOUR_SET], V_break[i-1, findfirst(isequal(d), DG_SET[:]), h] + 0.02 <= V_break[i, findfirst(isequal(d), DG_SET[:]), h])
    margin = 0
    # @constraint(OPF, [i = 1:5, d in DG_SET, h in HOUR_SET], V_break[i, findfirst(isequal(d), DG_SET[:]), h] + margin <= V_break[i+1, findfirst(isequal(d), DG_SET[:]), h])
    # @constraint(OPF, [i = 2:5, d in DG_SET, h in HOUR_SET], Vb_lim[i,1] <= V_break[i, findfirst(isequal(d), DG_SET[:]), h])
    # @constraint(OPF, [i = 2:5, d in DG_SET, h in HOUR_SET], V_break[i, findfirst(isequal(d), DG_SET[:]), h] <= Vb_lim[i,2])

    @constraint(OPF, [d in DG_SET, h in HOUR_SET, m in QUARTER_SET], v[d,h,m] == sum(λ[i, d, h, m] * V_break[i, findfirst(isequal(d), DG_SET[:]), h] for i in 1:6))
    ...
    #----------------------------------------------------------------------------------------------------------
        # Polyhedral
    #----------------------------------------------------------------------------------------------------------
    k = 16  # Number of linear constraints
    γ = π / k  # Angle step

    # polyhedral norm constraints
    for l in 1:k  # Loop over polygon constraints
        θ = l * γ  # Compute angle for the constraint
    ...
    # @constraint(OPF, [d in 1:dg_number, h in HOUR_SET, m in QUARTER_SET], Qdg[DG_SET[d], h, m] <= 0.44 * Sdg_max[DG_SET[d]])
    #----------------------------------------------------------------------------------------------------------
    #----------------------------------------------------------------------------------------------------------
    ...
    @constraint(OPF, PV_curt[DG in DG_SET, h in HOUR_SET, m in QUARTER_SET], PVC[DG,h,m] ==
                                                                                    (Pdg_max_vary[DG][h,m]-Pdg[DG, h, m]) )
    # @constraint(OPF, Fairness[DG in DG_SET, h in HOUR_SET, m in QUARTER_SET], PVC[h,m] >=
    #                                                                                         (Pdg_max_vary[DG][h,m]-Pdg[DG, h, m]) )
    ...
    # @objective(OPF, Min, sum(PVC[h,m] for h in HOUR_SET, m in QUARTER_SET))
    # @objective(OPF, Min, sum(PVC[h,m] for h in HOUR_SET, m in QUARTER_SET)
    ...
    # Γ = 1-α-β
    β = 1-α
    # @constraint(OPF, sum(PVC[h,m] for h in HOUR_SET, m in QUARTER_SET) <= 5)
    ...
    f1 = 1.2399546    # Final PVs - Fixed V_break + Max use PV
    f2 = 64.24629    # Final PVs - Fixed V_break + Max use PV

    # @objective(OPF, Min, α*sum(PVC[h,m] for h in HOUR_SET, m in QUARTER_SET)/f1 + β*v_varx/f2)
    @objective(OPF, Min, α*sum(PVC[DG,h,m] for DG in DG_SET, h in HOUR_SET, m in QUARTER_SET)/f1 +
    ...
    println("🟢 Iteration: $(it) - Optimization time: ", t_opt[it])
    ...
    global PVC_sum = value(sum(PVC[DG,h,m] for DG in DG_SET, h in HOUR_SET, m in QUARTER_SET))
    PVC_prnt = "PV Curt. : sum = " *string(round(PVC_sum, digits=7)) * " ➡️   " * string(round(PVC_sum/4, digits=7)) * " (p.u.) ➡️   " *
                    string(round(Sbase*PVC_sum/4, digits=7)) * " (kWh) ➡️   " *
                    string(round(100*PVC_sum/sum(Pdg_max_vary[DG][h,m] for DG in DG_SET, h in HOUR_SET, m in QUARTER_SET), digits=7)) * "(%)"
    println(PVC_prnt)
    ...
    DG_miss_prnt = "** DG_Miss/|DG|.|T|: " * string(round(DG_miss/(dg_number*length(HQ_SET)), digits=7)) * " (p.u.) ➡️   " *
                string(round(DG_miss_prcnt_av, digits=7)) * " (%) , at sunny hours: " * string(3*round(DG_miss_prcnt_av, digits=7)) * " (%)"
    ...
    if max_diff < tol
        ...
        println("🔸 Scenario: $(senario) ✅ Total optimization time: $(sum(t_opt)) seconds")
        break
    end
...
# println("🔸 Scenario: ", senario)
println("🟢 Opt. Times: ", t_opt)
println("🔵 Cumulative Opt. Time: ", t_opt_c)
println("🟡 MLE: ", dif_h)
