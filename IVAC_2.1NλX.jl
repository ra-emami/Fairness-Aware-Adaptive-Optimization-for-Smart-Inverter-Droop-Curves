...
# DG_SET = [7 17 31]
# DG_Psize = [1.4e6 1.1e6 1e6] # Max Pdg in w
# DG_Ssize = 1.1 .* DG_Psize # Max Sdg in VA
# DG_SIZE =  [DG_Psize;  # Max Pdg in w
#             DG_Ssize] # Max Sdg in VA
# DG_SET = [4 7 17 21 31]
...
# DG_SIZE =  [DG_Psize;  # Max Pdg in w
#             DG_Ssize] # Max Sdg in VA
DG_SET = [6 13 19 24 29 31]
DG_Psize = [0.8e6 0.75e6 0.55e6 0.6e6 0.5e6 0.4e6] # Max Pdg in w
DG_Ssize = 1.1 .* DG_Psize # Max Sdg in VA
DG_SIZE =  [DG_Psize;  # Max Pdg in w
            DG_Ssize] # Max Sdg in VA
...
for it in 1:max_iter
    ...
    # General knobs
    set_optimizer_attribute(OPF, "MIPGap", 0.005)  # Balance speed and accuracy (?% gap)
    ...
    # @constraint(OPF, [(i, j) in BRANCH_SET], Qloss[(i, j)] == ((v_r[i]-v_r[j])^2+(v_im[i]-v_im[j])^2) * X[i, j]/(R[i, j]^2+X[i, j]^2))
    # @constraint(OPF, [(i, j) in BRANCH_SET], Qloss[(i, j)] == ((v_r_pr[i,h,m] - v_r_pr[j])^2 + (v_im_pr[i,h,m] - v_im_pr[j])^2) * X[i, j] / (R[i, j]^2 + X[i, j]^2) +
    #                                                             (2 * (v_r_pr[i,h,m] - v_r_pr[j]) * X[i, j] / (R[i, j]^2 + X[i, j]^2)) * (v_r[i] - v_r_pr[i,h,m]) -
    #                                                             (2 * (v_r_pr[i,h,m] - v_r_pr[j]) * X[i, j] / (R[i, j]^2 + X[i, j]^2)) * (v_r[j] - v_r_pr[j]) +
    #                                                             (2 * (v_im_pr[i,h,m] - v_im_pr[j]) * X[i, j] / (R[i, j]^2 + X[i, j]^2)) * (v_im[i] - v_im_pr[i,h,m]) -
    #                                                             (2 * (v_im_pr[i,h,m] - v_im_pr[j]) * X[i, j] / (R[i, j]^2 + X[i, j]^2)) * (v_im[j] - v_im_pr[j]) )
    ...
    #                                                         2 * Ibr_r_pr[(i,j), h, m] * Ibr_r[(i,j), h, m] +
    #                                                         2 * Ibr_im_pr[(i,j), h, m] * Ibr_im[(i,j), h, m] -
    #                                                         (Ibr_r_pr[(i,j), h, m]^2 + Ibr_im_pr[(i,j), h, m]^2)))
    ...
    # # f3 = 1
    f1 = 0.0231253    #
    f2 = 35.5211879    # DG_SET = [6 13 19 24 29 31] # DG_Psize = [0.8e6 0.75e6 0.55e6 0.6e6 0.5e6 0.4e6] # Max Pdg in w
    # f3 = 1
