# =============================================================================
# Pareto study — Scenario 5 (variable breakpoints + proportional fairness)
#
...
#
# This script computes ONE point of the Pareto front between fairness-aware
# PV utilization and voltage regulation: set the weight α below and re-run
# for each α ∈ {0.1, 0.2, ..., 0.9} to trace the full front (Fig. 9 of the
# paper reports the front for Scenario 6).
#
# Reference:
...
for it in 1:max_iter
    ...
    global scenario = 5
    α = 0.5   # Pareto sweep weight: re-run with α ∈ {0.1, 0.2, ..., 0.9}
    β = 1-α

    f1 = 2.9255239    # η for the proportional fairness term (variable breakpoints)
    ...
    obj = round(value(α*sum(fairness_index[h,m] for h in HOUR_SET, m in QUARTER_SET)/f1 +
        β*sum(Δv[i,h,m] for i in BUS_SET, h in HOUR_SET, m in QUARTER_SET)/f2), digits=7)
