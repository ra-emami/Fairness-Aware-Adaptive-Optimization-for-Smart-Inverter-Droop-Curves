# =============================================================================
# Scenario 1 — Fixed Volt-Var breakpoints + maximum PV utilization
#
# Multi-period enhanced IVACOPF for smart-inverter dispatch on the 33-bus
# distribution feeder: 24-hour horizon at 15-minute resolution (96 steps).
# The six breakpoint voltages of each inverter's Q-V droop curve are fixed
# at the standard set {0.88, 0.90, 0.97, 1.00, 1.02, 1.10} p.u.
# Total PV curtailment is minimized without a fairness policy (Eq. 36).
#
# Reference:
...
for it in 1:max_iter
    ...
    @variable(OPF, 0 <= PVC[DG_SET, HOUR_SET, QUARTER_SET])  # per-unit PV curtailment (Eq. 36)

    # ------------------- Volt-Var droop: Lambda method with fixed breakpoints
    ...
    # Fixed breakpoint voltages {0.88, 0.90, 0.97, 1.00, 1.02, 1.10} p.u.
    V_break = [0.88, 0.90, 0.97, 1.00, 1.02, 1.10]

    # PCC voltage interpolation (Eq. 22); linear since the breakpoints are fixed
    @constraint(OPF, [d in DG_SET, h in HOUR_SET, m in QUARTER_SET], v[d,h,m] == sum(λ[i, d, h, m] * V_break[i] for i in 1:6))
    ...
    # ---------------------- PV utilization without fairness (Eq. 36)
    # Ψ is the total curtailment summed over all PV units and time steps
    ...
    f1 = 1.2399546    # η for the total-curtailment term (fixed breakpoints)
    f2 = 64.24629     # η for the voltage-deviation term (fixed breakpoints)
    ...
    global PVC_sep = Dict(DG => [value(sum(PVC[DG,h,m] for h in HOUR_SET, m in QUARTER_SET))] for DG in DG_SET)
    ...
    println("Total curtailment term (Eq. 36): ", round(PVC_sum, digits=7), " p.u.")
